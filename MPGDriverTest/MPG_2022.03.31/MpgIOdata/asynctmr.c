...
/*----------------------------------------------------------------------------*/
/*    Copyright (c) National Instruments 1999-2020.  All Rights Reserved.     */
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Title:       asynctmr.c                                                    */
...
static void RemoveAsyncTimer(tAsyncTimerRec *asyncTimerPtr)
{
	...
	// Wait for outstanding timer callback to finish
	while (1)
	{
		if (!asyncTimerPtr->inCallback)
			break;
		CmtReleaseLock(asyncLock);
		Sleep(150);
		CmtGetLock(asyncLock);
...
static void CVICALLBACK LocalTimerCallbackFunc (void *data)
{
	...
	// Create local copies of data used to call the callback.
	// This data can be modified via SetAsyncTimerAttribute
	// between releasing asyncLock and actually calling the
	// callback (see below).
	AsyncTimerCallbackPtr timerCB = asyncTimerPtr->callback;
	void* timerCBData = asyncTimerPtr->callbackData;
	int timerUserHandle = asyncTimerPtr->userHandle;

	asyncTimerPtr->inCallback = 1;
	...
	if (timerCB)
#if defined (_NI_mswin64_) && _NI_mswin64_
		(*timerCB) ((int)0, timerUserHandle,
			(int)EVENT_TIMER_TICK, timerCBData, 0, 0);
#else
		(*timerCB) ((int)0, timerUserHandle,
			(int)EVENT_TIMER_TICK, timerCBData,
			(int) &timerTime, (int) &deltaTime);
#endif
